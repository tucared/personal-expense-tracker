locals {
  ...
  module_name = "gsheets-pipeline" # Avoid underscores in the name
}
...
## Creating a service key for data_bucket_writer_service_account_email and saving it in a Google Secret

resource "google_service_account_key" "data_bucket_writer" {
  service_account_id = var.data_bucket_writer_service_account_email
}

resource "google_secret_manager_secret" "data_bucket_writer_key" {
  secret_id = "SOURCES__GOOGLE_SHEETS__PRIVATE_KEY"

  labels = {
    application = "google_sheets_pipeline"
  }
...
resource "google_secret_manager_secret_version" "data_bucket_writer_key" {
  secret      = google_secret_manager_secret.data_bucket_writer_key.id
  secret_data = jsondecode(base64decode(google_service_account_key.data_bucket_writer.private_key))["private_key"]
}

resource "google_secret_manager_secret_iam_member" "data_bucket_writer_key" {
  project   = var.project_id
  secret_id = google_secret_manager_secret.data_bucket_writer_key.id
...
resource "google_cloudfunctions2_function" "this" {
  ...
  build_config {
    ...
    entry_point = "google_sheets_pipeline"
  }
  ...
  service_config {
    ...
    # Add the secret to the environment variables
    secret_environment_variables {
      project_id = var.project_id
      key        = "SOURCES__GOOGLE_SHEETS__PRIVATE_KEY"
      secret     = google_secret_manager_secret.data_bucket_writer_key.secret_id
      version    = "latest"
    }
  }
}
